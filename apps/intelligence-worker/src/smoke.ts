...
import { buildAgentContext, HostedLlmIntelligence } from "@tomeet/intelligence";
...
const baseContext = buildAgentContext([], baseModel);
...
if (!feedbackInsight.currentIntent) {
  throw new Error("真实模型未完成反馈意图整理");
}
...
const candidates = Array.from({ length: 5 }, (_, index) => {
  ...
  return {
    ...
    matchingNarrative: index === 4
      ? "观察细腻，愿意先共同做一件事再自然打开话题，偏好有空间感的相处。"
      : `第 ${index + 1} 位用户愿意在共同体验中自然回应他人。`
  };
});
