...
- `UserMemoryProfile`：可从 active memory 重建的隐藏 `profileNarrative` 与 `matchingNarrative`。
- `CurrentIntent`：用户当前的社交意图。
- `SocialHistory`：历史匹配和线下游戏。
- `FeedbackMemory`：兼容保留的活动反馈摘要。
- `MultimodalUnderstanding`：有界的图片/短录音近期理解；不会自动成为稳定事实。

默认上下文不再注入完整 `user_models`。详细策略见 [Agent Memory / Context V2](docs/agent-memory-context.md)。
...
- [Agent Memory / Context V2](docs/agent-memory-context.md)
- [API 契约](docs/api.md)
