...
`WECHAT_WEB_REGISTRATION_URL` 是首次微信开场白中的个性化注册链接。只有扫码前
尚未对应数据库用户的微信身份才会创建欢迎 outbox；API 同时创建同一 UUID 的
Supabase 匿名账号，并把完整开场白与一次性链接加密写入该任务。首次入站握手只负责
解锁任务且不进入 Agent，outbox 随后发送完整开场白与链接。生产 Supabase
