...
The activation welcome appends two bubbles:

```text
想在网页上和别人线下加好友吗，有机会上TOMEET“必吃榜”！

点这里完成注册：https://tomeet.chat/register#claim=<one-time-token>
```
...
- The page title is `创建你的 TOMEET 账号`.
- Explain that registration automatically links the current WeChat and keeps
  its conversation and profile.
- If the browser already has a different Supabase user, require an explicit
  account-switch confirmation before redeeming the claim.
