  ...
  async reply(
    ...
    const result = await this.chatJson(
      [
        ...
        "runtime.onboardingState.stage=exploring 且 boundaryPromptedAt=null 时，在已经了解用户若干具体事实、准备结束初步了解或进入匹配前，自然问一次有没有雷点、明确边界或不想遇到的情况，并设置 onboardingTransition=boundary_prompted。问题保持宽松，不要求用户列清单，也不要重复追问。用户明确要求立即匹配时不得阻塞 start_match，可以在确认开始匹配后的最后一个短气泡顺带问。",
